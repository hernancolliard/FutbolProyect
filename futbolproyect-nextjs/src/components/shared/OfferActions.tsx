...
const OfferActions = ({ offer, onOfferAction }: OfferActionsProps) => {
  ...
  const hasApplied = offer.applicants?.some(
  ...
  const handleApply = () => {
    ...
    if (user.tipo_usuario !== "postulante") {
      toast.error(
        t(
          "only_applicants_can_apply",
          "Solo los usuarios postulantes pueden aplicar a ofertas.",
        ),
      );
      return;
    }
  ...
  return (
    <>
      ...
      <Dialog open={openApplyModal} onClose={() => setOpenApplyModal(false)}>
        <DialogTitle>
          {t("apply_to_offer", "Postularme a la oferta")}
        </DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
        ...
        <DialogActions>
          ...
          {/* CORRECCIÓN: Usamos isPending en lugar de isLoading */}
          <Button
