...
import { Typography, Stack, CircularProgress, Alert, Card, Button } from '@mui/material';
...
const fetchUserApplications = async (userId: number): Promise<Application[]> => {
...
export default function MyApplicationsSection({ userId }: MyApplicationsSectionProps) {
                ...
                <Stack spacing={2}>
                    {applications.map((app) => (
                        <Card key={app.id} variant="outlined" sx={{ p: 2 }}>
                            ...
                            <Typography variant="body2">
                                <strong>{t('date', 'Fecha')}:</strong> {new Date(app.fecha_postulacion).toLocaleDateString()}
                            </Typography>
                            <Button
                                component={Link}
