...
const VideoFormModal = ({ open, onClose, video, onSave }) => {
  ...
  const { mutate, isPending: isSaving } = useMutation({
    ...
    onError: (err) => {
      const errorMessage = err.message || t('video_save_error', 'Error al guardar el video.'); // Access err.message
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
      <DialogTitle>{video ? t('edit_video', 'Editar Video') : t('add_video', 'Añadir Video')}</DialogTitle>
      <DialogContent>
        <form onSubmit={handleSubmit}>
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <TextField
              name="title"
              label={t('video_title', 'Título del Video')}
            ...
            <TextField
              name="youtube_url"
              label={t('youtube_link', 'Enlace de YouTube')}
            ...
            <TextField
              select
              name="position"
              label={t('video_position', 'Posición del Video')}
