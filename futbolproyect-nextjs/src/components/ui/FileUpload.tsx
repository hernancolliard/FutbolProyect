...
function FileUpload({ onFilesChange, uploadProgress = 0, multiple = false, initialFiles = [] }: FileUploadProps) {
  ...
  const onDrop = useCallback((acceptedFiles: File[], fileRejections: FileRejection[]) => {
    const newFiles: CustomFile[] = acceptedFiles.map(file => Object.assign(file, {
  ...
  return (
    <Box>
      <DropzoneContainer {...getRootProps()} isDragActive={isDragActive}>
        <input {...getInputProps()} />
        <Typography>{t('drag_drop_files', 'Arrastra y suelta imágenes aquí, o haz clic para seleccionar')}</Typography>
        ...
        <List sx={{ mt: 2 }}>
          {files.map((file, i) => (
            ...
            >
              ...
              <ListItemText primary={file.name} secondary={file.size ? `${(file.size / 1024).toFixed(2)} KB` : ''} />
            </ListItem>
          ))}
        </List>
      )}
