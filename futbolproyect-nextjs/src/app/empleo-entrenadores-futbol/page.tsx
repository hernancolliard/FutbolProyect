...
async function getCoachOffers(): Promise<Offer[]> {
  ...
  const res = await fetch(`${apiBaseUrl}/offers?puesto=entrenador`, { next: { revalidate: 3600 } }); // Filter by position
  if (!res.ok) {
...
const EmpleoEntrenadoresFutbolPage = async () => {
  ...
  const pageContent = {
    h1: translations['empleo_entrenadores_futbol_h1'] || "Empleo para entrenadores de fútbol",
    mainText: translations['empleo_entrenadores_futbol_main_text'] || `Los entrenadores de fútbol cumplen un rol central en el desarrollo deportivo de jugadores y equipos. En FutbolProyect creamos un espacio específico donde entrenadores y cuerpos técnicos pueden encontrar oportunidades laborales en clubes y academias.
...
Si sos entrenador y estás buscando crecer profesionalmente, contar con un perfil visible en una plataforma dedicada al fútbol aumenta significativamente tus posibilidades. FutbolProyect conecta talento con proyectos deportivos reales.`,
    h2: translations['empleo_entrenadores_futbol_h2'] || "Trabajo para entrenadores de fútbol",
    ctaText: translations['empleo_entrenadores_futbol_cta'] || "Registrate y accedé a empleo para entrenadores de fútbol",
    ctaLink: "/register"
  };
