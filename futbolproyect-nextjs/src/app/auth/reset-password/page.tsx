    ...
    <Box sx={{ mt: 4, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
      <Card sx={{ maxWidth: 400, width: '100%', p: 2 }}>
        <CardContent>
          ...
          {!token ? (
            <Alert severity="error" sx={{ mt: 2 }}>{t('invalid_token_or_expired', 'Token inválido o expirado.')}</Alert>
          ) : (
            <form onSubmit={handleSubmit}>
              <Stack spacing={2} sx={{ mt: 2 }}>
                ...
                <TextField
                  type="password"
                  id="confirmPassword"
                  label={t('confirm_new_password_label', 'Confirmar Nueva Contraseña')}
                ...
                <Button type="submit" variant="contained" color="primary" disabled={loading} fullWidth>
                  {loading ? <CircularProgress size={24} /> : t('update_password_button', 'Actualizar Contraseña')}
                </Button>
              </Stack>
            </form>
          )}
