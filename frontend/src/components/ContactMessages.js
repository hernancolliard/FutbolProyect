...
function ContactMessages() {
  ...
  const fetchMessages = async () => {
    try {
      ...
      setError(err.message || t('contact_messages_fetch_error'));
      toast.error(t('contact_messages_fetch_error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleReplySubmit = async () => {
    if (!replyText.trim()) {
      toast.warn(t('reply_message_empty'));
      return;
    }
    ...
    try {
      ...
      toast.success(t('reply_sent_success'));
      handleCloseModal();
      fetchMessages(); // Refresh messages
    } catch (err) {
      toast.error(err.response?.data?.message || t('reply_sent_error'));
    } finally {
