...
function Header({ onShowLoginModal, onShowRegisterModal }) {
  ...
  const handleLanguageMenuOpen = (event) => { // Nueva función para abrir el menú de idioma
    setLanguageAnchorEl(event.currentTarget);
  };

  const handleLanguageMenuClose = () => { // Nueva función para cerrar el menú de idioma
  ...
  return (
    <AppBar position="fixed" color="primary" sx={{ height: '80px', justifyContent: 'center' }}>
      <Toolbar sx={{ display: 'flex', justifyContent: 'space-between', bgcolor: 'primary.main' }}>
        ...
        <Box sx={{ display: { xs: 'flex', md: 'none' } }}>
          ...
          >
            ...
            <MenuItem component={Link} to="/offers" onClick={handleMobileMenuClose}>
              {t('offers')}
            </MenuItem>
            ...
            {user ? (
              <>
                ...
                <MenuItem component={Link} to={`/profile/${user.id}`} onClick={handleMobileMenuClose}>
                  {t('my_profile')}
                </MenuItem>
                <MenuItem disabled>| {t('welcome_user', { name: user.name })}</MenuItem>
