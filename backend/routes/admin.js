...
router.post("/users/:id/grant-subscription", [verificarToken, verificarAdmin], async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: "Server error while granting subscription." });
  }
});
...
router.delete("/offers/:id", [verificarToken, verificarAdmin], async (req, res) => {
  const { id } = req.params;
  try {
...
router.put("/subscriptions/:id", [verificarToken, verificarAdmin], async (req, res) => {
  const { id } = req.params;
  const { price_usd, price_mp } = req.body;

  if (price_usd === undefined || price_mp === undefined) {
    return res.status(400).json({ message: "price_usd and price_mp are required." });
  }
